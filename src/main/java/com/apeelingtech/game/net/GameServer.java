	...
	private void parsePacket(byte[] data, InetAddress address, int port) {
		...
		switch (type) {
			...
			case LOGIN:
				...
				PlayerMP player = new PlayerMP(gameScreen.level, gameScreen, 100, 100, ((Packet00Login) packet).getUsername(), address, port, ((Packet00Login) packet).getShirtColor(), ((Packet00Login) packet).getSkinColor(), ((Packet00Login) packet).getCharacterType());
	...
	public void addConnection(PlayerMP player, Packet00Login packet) {
		...
		for (PlayerMP p : this.connectedPlayers) {
			if (player.getUsername().equalsIgnoreCase(p.getUsername())) {
				...
				// exists
				packet = new Packet00Login(p.getUsername(), p.x, p.y, p.getShirtColor(), p.getSkinColor(), p.getCharacterType());
