package com.apeelingtech.game.net;
...
import com.apeelingtech.game.net.packets.Packet;
...
import com.apeelingtech.game.net.packets.Packet.PacketTypes;
...
public class GameClient extends Thread {
	...
	private void parsePacket(byte[] data, InetAddress address, int port) {
		...
		switch (type) {
			...
			case DISCONNECT:
				...
				gameScreen.level.removePlayerMP(((Packet01Disconnect) packet).getUsername());
				break;
			case MOVE:
	...
	private void handleLogin(Packet00Login packet, InetAddress address, int port) {
		...
		PlayerMP player = new PlayerMP(gameScreen, false, packet.getX(), packet.getY(), packet.getUsername(), address, port, packet.getShirtColor(), packet.getSkinColor(), packet.getCharacterType());
		gameScreen.level.addEntity(player);
	}
	
	private void handleMove(Packet02Move packet) {
		this.gameScreen.level.movePlayer(packet.getUsername(), packet.getX(), packet.getY(), packet.getNumSteps(), packet.isMoving(), packet.getMovingDir());
	}
}
